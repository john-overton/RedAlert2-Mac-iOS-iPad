...
test('observer Enter and Backspace both force observer audience', () => {
    for (const key of ['Enter', 'Backspace']) {
        const f = fixture(true);
        expect(f.press(key)).toBe(true);
        expect(f.history.lastComposeTarget.value.name).toBe('#observers');
        f.handler.endTyping();
        expect(f.messages.isComposing).toBe(false);
    }
});
