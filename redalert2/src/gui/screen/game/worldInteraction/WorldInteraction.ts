...
export class WorldInteraction {
    ...
    private readonly handleMouseUp = (event: any): void => {
        ...
        if (event.cancelled) {
            // Aborted touch gesture: release every bit of held state, but never
            // execute a click/order from it.
            if (this.mousePressed === undefined && this.minimapDragButton === undefined) {
                return;
            }
